---
title: How to process a Giotto object?
output:
  html_document:
    toc: yes
    toc_depth: '2'
...
This dataset contains multiple field of views; here, we illustrate a workflow to bring the FOVs together for holistic analysis, and then subset the Giotto Object to analyze the cortex. 
...
Since **subsetGiotto** returns a Giotto object, multiple different objects may be created to store subsets. For the purposes of this tutorial, we will focus only on the cortex FOVs, which is why the original Giotto Object has been overwritten upon calling **subsetGiotto**.
...
```
![](../inst/images/getting_started_figs/data_processing/0-filterDistributions_cells.png){ width=50% }


```{r, eval=FALSE}
 filterDistributions(testobj, detection = 'feats')
```
![](../inst/images/getting_started_figs/data_processing/1-filterDistributions_feats.png){ width=50% }
...
```
![](../inst/images/getting_started_figs/data_processing/2-filterCombinations.png){ width=50% }
...
The 'pearson_resid' method uses the Lause/Kobak et al. method. Fist, expected values are calculated based on Pearson correlations. Next, z-scores are calculated based on observed and expected values.

It should be noted that normalizing with this method will save the data within the "scaled" expression slot,
NOT the "normalized" slot.
...
Since the data in this tutorial is not in-situ RNA data, we will not show this method here. It may be utilized by specifying the norm_methods argument as 'osmFISH'.
...
```{r, eval=FALSE}
# Add gene & cell statistics to the Giotto object using the data normalized with method A (above)
...
**processGiotto** completes the filtering, normalization, statistical, and adjustment steps of processing in one single step, ideal for faster processing. 

Since adjustment is not necessary for every dataset, it may be set to NULL to skip this processing step. All other arguments are user-determined; default arguments will apply to all steps if no arguments are provided.
```{r, eval=FALSE}
