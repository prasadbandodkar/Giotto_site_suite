...
This tutorial walks through the dimension reduction and clustering capabilities of Giotto. We begin this tutorial with the Giotto Object processed in the previous tutorial, [data processing](./data_processing.html). For convenience, the code to create this object is provided below. However, we encourage new users to review the data processing tutorial prior to beginning this tutorial.
...
```
![](../inst/images/getting_started_figs/dimension_reduction/0-HVFplot_covgroups.png){ width=50% } 
```{r, eval=FALSE}
# Calculate HVF using variance of Pearson residuals
testobj <- calculateHVF(gobject = testobj, method = 'var_p_resid')

```
![](../inst/images/getting_started_figs/dimension_reduction/1-HVFplot_varpresid.png){ width=50% } 
```{r, eval=FALSE}
#calculate HVF using the loess regression prediction model
testobj <- calculateHVF(gobject = testobj, method = 'cov_loess')

```
![](../inst/images/getting_started_figs/dimension_reduction/2-HVFplot_covloess.png){ width=50% } 

PCA can be run based on the highly variable genes. After PCA, a tSNE, a UMAP, or both may be run. 

For this example, highly variable genes have been identified using Loess Regression predictions.
```{r, eval=FALSE}
...
## 2. Clustering
Clustering  cells into distinct groups based on feature expression. The input is a Giotto object that has undergone PCA dimension reduction and thereafter either tSNE or UMAP dimension reduction.
```{r, eval=FALSE}
...
testobj <- doLouvainCluster(gobject = testobj, version = 'community', resolution = 0.4)
...
Clusters of interest can be further sub-clustered, choose your clusters with **selected_clusters**. Note that the same HVF selection and PCA parameters are used for subclustering for consistency.
```{r, eval=FALSE}
