...
By providing values to other **createGiottoObject** parameters, it is possible to add:
 - **Cell** or **feature (gene) metadata**
     - See [**addCellMetadata**](http://giottosuite.com/reference/addCellMetadata.html) and [**addFeatMetadata**](https://giottosuite.com/reference/addFeatMetadata.html)
 - **Spatial networks** or **grids**
     - See [Visualizations](./Visualizations.html)
 - **Dimension reduction**
     - See [Clustering](./dimension_reduction.html)
 - **Images** 
     - See [Imaging](./getting_started_images.html)
 - **giottoInstructions**
     - See [**createGiottoInstructions**](http://giottosuite.com/reference/createGiottoInstructions.html) and below
...
Alternatively, a named list may also be provided to the *instructions* argument of **createGiottoObject**. However, ensure that all arguments to [**createGiottoInstructions**](http://giottosuite.com/reference/createGiottoInstructions.html) are defined when providing instructions as a named list, since default values are only applied to instructions when made with **createGiottoInstructions**. *Note that python_path must be specified when providing instructions as a named list, and may not be provided as NULL.*
...
See [showSaveParameters](http://giottosuite.com/reference/showSaveParameters.html) and the [Saving Options](./getting_started_saving.html) tutorial for alternative methods to save plots.

```{r, eval=FALSE}
# Plot according to Giotto Instructions (default)
spatPlot(custom_gobject)

# Plot clusters, create, and save to a new subdirectory, all while overwritting formatting
spatPlot(custom_gobject, 
...
```
![](../inst/images/getting_started_figs/getting_started_gobject/cell_clusters.png)
