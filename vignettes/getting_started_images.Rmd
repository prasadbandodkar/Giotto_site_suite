...
<br>
Multiple images can be attached to your Giotto object. Spatial data can be overlaid over these images in order to gain further context. Additionally, images themselves are also a form of spatial information and the images themselves can also be used for downstream analyses. While Giotto attempts to automate the addition and alignment of images as much as possible, manual adjustments may sometimes be necessary, and this tutorial will be covering both topics.  
Visium's normal human prostate FFPE [dataset](https://www.10xgenomics.com/resources/datasets/normal-human-prostate-ffpe-1-standard-1-3-0) dataset will be used as an example for both the Visium and manual adjustment workflows.  
Additionally the Nanostring CosMx lung data will be used as an example of working with high definition images that may require ROI alignment and or stitching.


### Conceptual Overview
Giotto currently supports two types of image objects. For most purposes, images are loaded in through the [*terra*](https://rdrr.io/cran/terra/) package and then placed into **giottoLargeImage** objects to be worked with in Giotto. **giottoLargeImage** objects only load in sampled subsets of the original image that are needed to represent the image, making it very efficient for working with very high-resolution images.    
    
Smaller images can also be loaded in through the [*magick*](https://rdrr.io/cran/magick/) package and then placed into **giottoImage** objects. This offers direct access to the powerful image processing functionality in the *magick* package, but direct usage of this image class is mostly legacy. **giottoLargeImage** objects can be downsampled into **giottoImage** objects when necessary through use of the **convertGiottoLargeImageToMG()** function.
